...
$(function(){
  ...
  if ($('#genba_id').val() == '') {
    ...
    var tr_count = $('.wk_taiin0').val().split(',')
    var count = tr_count[0]
    for (var j=0;j<count;j++) {
      var wk_taiin = $('.wk_taiin'+j).val().split(',')
      // var wk_no = wk_taiin[1]
      // var taiin_id = wk_taiin[2]
      // for (var i=1;i<=31;i++) {
        var j_flg = true
        var datalist = {
          j_flg: j_flg,
        ...
        .done(function(data){
          ...
          for (var i=1;i<=31;i++) {
            ...
            // console.log(i,day)
            if (!data.detail) {
              $('.tr'+j+' .td'+i+' a').attr('onclick','regist('+datalist.wk_no+')')
              $('.tr'+j+' .td'+i+' a').text('-')
              continue
            }
            for (kinmu of data.detail) {
              if (day == kinmu.plan_date.substr(8,2)) {
                // console.log(kinmu)
                $('.tr'+j+' .td'+i+' a').attr('name','del')
                $('.tr'+j+' .td'+i+' a').attr('onclick','del('+kinmu.detail_no+')')
                $('.tr'+j+' .td'+i+' a').text(kinmu.plan_kbn_name+kinmu.plan_hosoku)
                break
              } else {
                $('.tr'+j+' .td'+i+' a').attr('onclick','regist('+datalist.wk_no+')')
                $('.tr'+j+' .td'+i+' a').text('-')
  ...
  */
  ...
  $('.td').click(function(){
    // const genba_id = $('#genba_id').val()
    // const nengetu = $('#nengetu').val()
    // const taiin_id = $(this).children('input').val()
    // const click = $(this).children('a').attr('onclick')
    var hrefarray = $(this).children('a').prop('href').split('#')
    // const inputarray = $(this).parent().find('input').val().split(',')
    var inputarray = $(this).children('input[name="detail"]').val().split(',')
    var onclickarray = $(this).children('a').attr('onclick').split(/[(,)]/)
    // const wk_no = array[1]
    // console.log(inputarray)
    // return
    var datalist = {
      // click: $(this).children('a').attr('onclick'),
      click: onclickarray[0],
      shift_no: $('#shift_no').val(),
      genba_id: $('#genba_id').val(),
      nengetu: $('#nengetu').val(),
      // wk_no: hrefarray[1],
      wk_no: onclickarray[1],
      detail_no: '',
      // taiin_id: $(this).parent().find('input').val()
      taiin_id: inputarray[0],
      day: '',
      token: $('#token').val(),
      kensyu: $('#kensyu:checked').val()
    }
    // console.log(datalist)
    // return
    if (datalist.click == 'regist') {
    // if (datalist.click.match(/regist/)) {
      // console.log(datalist)
      // return
      if (!datalist.shift_no) {
        alert('登録するシフトを選択してください')
        return
      }
      datalist.day = onclickarray[2]
      // console.log(datalist)
      // return
      // insertdetail(datalist)
      $.ajax({
        url : "kinmuyotei.php",
        type:"post",
        data: datalist,
      ...
      .done(function(data){
        console.log("OK",data)
        // $(this).rermoveAttr('style')
        $(this).children('a').attr({
          onclick:'del('+data.detail_no+')',
          name:'del'
        })
        if (data.kensyu != '') {
          $(this).attr('bgcolor','#CCFF99')
        }
        if (data.shift_color != '' && data.shift_color != ' ') {
          $(this).attr('bgcolor',data.shift_color)
        }
        $(this).children('input[name="wk_no0"]').remove()
        $(this).children('a').text('')
        if (data.plan_hosoku != '') {
          $(this).children('a').append('<font size="-1">'+data.plan_kbn_name+'<br>'+data.plan_hosoku+'</font>')
        } else {
          $(this).children('a').append('<font size="-1">'+data.plan_kbn_name+'</font>')
        }
        $('#token').val(data.token)
        // $(this).children('a').text(data.plan_kbn_name)
      })
      .fail(function(data){
        console.log("NG")
      })
      ...
      // console.log(datalist)
      // return
      datalist.detail_no = onclickarray[1]
      datalist.delete2 = true
      updeldetail(datalist)
      $.ajax({
        url : "kinmuyotei.php",
        type:"post",
        data: datalist,
      ...
      .done(function(data){
        ...
        if (data.flg == '1') {
          ...
          $(this).children('a').text('－')
          $(this).children('a').attr({
            onclick:'regist('+hrefarray[1]+','+inputarray[1]+')',
            name:'regist'
          })
          $(this).children('a').after($('<input>',{
            type:'hidden',
            name:'wk_no0',
            value:''
          }))
      ...
      // $(this).children('a').attr({
      //   onclick:'regist('+datalist.wk_no+','+inputarray[1]+')',
      //   name:'regist'
      // })
    }
    // console.log(datalist)
  ...
  var cnt = '';
  var device = '<?php print($common->device); ?>';
  var genba_id = '<?php print($genba_id); ?>';
  var td1 = '';
  var td2 = '';
  var td3 = '';
  var td4 = '';
  ...
  //getWidth()
      ...
      if (device == 'mobile' && genba_id == '') {
          if (size < 400) {
              cnt = 'y';
              td1 = $('.head table td:eq(2)').remove()
              td2 = $('.head table td:eq(2)').remove()
              td3 = $('.head table td:eq(2)').remove()
              td4 = $('.head table td:eq(2)').remove()
              td5 = $('.search_button').remove()
              
              $('.head table td:eq(1)').prop('colspan','2')
              $('.head table').append($('<tr>'))
              $('.head table tr:eq(1)').append(td1,td2,td3)
              $('.head table').append($('<tr>'))
              $('.head table tr:eq(2)').append(td4,td5)
          } else {
              cnt = 'x';
          }
      }
  ...
  //        if (cnt != 'x') {
  //            cnt = 'x';
  //            $('.head table td:eq(1)').removeAttr('colspan')
  //            $('.head table tr:eq(1)').remove()
  //            $('.head table tr:eq(1)').remove()
  //            $('.head table tr').append(td1)
  //            $('.head table tr').append(td2)
  //            $('.head table tr').append(td3)
  //            $('.head table tr').append(td4)
  //            $('.head tr:first').append(td5)
  //            
  //            inputClick()
  //        }
  //        }
...
function insertdetail(datalist) {
  // console.log(datalist)
  // return
  $.ajax({
    url : "kinmuyotei.php",
...
var chk_kbn = $('#chk_kbn').val();
    switch(mode) {
        case 1:			// 印刷
            winprint=window.open('kinmuyotei.php?search=&dispctl=1&genba_id=' + genba_id + '&nengetu=' + nengetu + "&chk_kbn=" + chk_kbn,'勤務予定表','width=1000,height=1000,toolbar=0,location=0,directories=0,status=0,menubar=1,scrollbars=1,resizable=1');
            winprint.print(); 
            break;
    }
}
...
function up(wk_no) {
  ...
  location.href = "kinmuyotei.php?search=&up=&wk_no=" + wk_no + "&genba_id=" + genba_id + "&nengetu=" + nengetu + "&pos=" + pos + "&chk_kbn=" + chk_kbn;
}
...
function down(wk_no) {
  ...
  location.href = "kinmuyotei.php?search=&down=&wk_no=" + wk_no + "&genba_id=" + genba_id + "&nengetu=" + nengetu + "&pos=" + pos + "&chk_kbn=" + chk_kbn;
}
...
function confirm1(no,taiin_id,name) {
  ...
  location.href = "kinmuyotei.php?search=&delete1=&wk_no=" + no + "&taiin_id=" + taiin_id + "&genba_id=" + genba_id + "&nengetu=" + nengetu +
    "&shift_no=" + shift_no + "&chk_kbn=" + chk_kbn;;
}
...
function confirm2() {
  ...
  var nengetu = $('#nengetu').val();
  var shift_no = $('#shift_no').val();
  var conf_flg = $('#conf_flg').val();
  if (!confirm('既に予定確定データがあります、更新してもよろしいですか?')) return false;
  location.href = "kinmuyotei.php?search=&conf=&genba_id=" + genba_id + "&nengetu=" + nengetu + "&conf_flg=" + conf_flg;
}
...
function regist(wk_no, id) {
  ...
  if (shift_no == "") {
    ...
    location.href = "kinmuyotei.php?search=&regist=&genba_id=" + genba_id + "&nengetu=" + nengetu + "&shift_no=" +
      shift_no + "&wk_no=" + wk_no + "&id=" + id + "&kensyu=" + kensyu + "&pos=" + pos + "&token=" + token + "&chk_kbn=" + chk_kbn;;
  }
}
...
function ins() {
  ...
  if (staff_id == "") {
    ...
    location.href = "kinmuyotei.php?search=&ins=&genba_id=" + genba_id + "&nengetu=" + nengetu + "&shift_no=" +
      shift_no + "&staff_id=" + staff_id + "&kensyu=" + kensyu + "&chk_kbn=" + chk_kbn;
  }
}
...
function del(wk_no) {
  ...
  location.href = "kinmuyotei.php?search=&delete2=&genba_id=" + genba_id + "&nengetu=" + nengetu + "&shift_no=" +
    shift_no + "&wk_no=" + wk_no + "&pos=" + pos + "&kensyu=" + kensyu + "&token=" + token + "&chk_kbn=" + chk_kbn;;
}
</script>
    ...
    <form name="frm" id="frm" method="POST" action="kinmuyotei.php">
      ...
      <div class="row">
        <div class="col-xs-12 col-md-12">
          <table style="border-collapse:collapse;" class="head">
            <tr>
              <td style="border:1px; border-style: solid;">
                <table>
                  <tr>
                    ...
                    <td id="nen" bgcolor="#d5d5d5"  width=110>
                    
                      <input type="hidden" class="form-control nengetu_t" id="nengetu" name="nengetu" maxlength="6" value="<?php print($nengetu); ?>" <?php print($disabled); ?>>
                      <select id="nengetu" class="form-control nengetu_s" name="nengetu" <?php print($disabled); ?>>
                    ...
                    <td>
                      <label class="m-0">
                        <input type="checkbox" id="jikan" class="d-inline-block" name="jikan" value="1" <?php print($disabled); if ($jikan != "") {print("checked");} ?>><span class="d-inline-block p-1">時間<br>表示</span>
                      </label>
                    </td>
                    <?php } ?>
                  </tr>
              ...
              <?php /*
              <td style="border:1px; border-style: solid;" bgcolor="">
                <table>
                  <tr>
                    <td class="p-1">勤務<br>体系</td>
                  </tr>
                </table>
              </td>
              <td style="border:1px; border-style: solid;">
...
?>
              ...
              */ ?>
              ...
              <!-- 印刷表示以外 -->
              <?php if ($dispctl == "" && $genba_id != "") { ?>
              <td>
                  <?php if ($conf_flg == 1) { ?>
                      <a href="kinmuyotei.php?search=&conf=&genba_id=<?php print($genba_id); ?>&nengetu=<?php print($nengetu); ?>&conf_flg=<?php print($conf_flg); ?>" class="btn btn-success btn-block" role="button" aria-pressed="true">予定の確定</a>
                  <?php } elseif ($conf_flg == 2) { ?>
                      <a class="btn btn-success btn-block" role="button" aria-pressed="true" onClick=confirm2() style=color:white;>予定の確定 (確定済)</a>
                  <?php } else { ?>
                      <a href="#" class="btn btn-dark disabled1 btn-block" role="button" aria-pressed="true">予定の確定 (確定済)</a>
                  <?php } ?>
              </td>
              <?php if ($common->device == "pc") { ?>
              <td class="px-5"><input type="button" class="btn btn-warning btn-block" role="button" name="print" value="印刷" onClick="FJS_Location(1,<?php print($nengetu); ?>)"></td>
              <?php } ?>
              <td <?php echo $common->device != "pc" ? "class='pl-5'" : "" ; ?>><button type="submit" class="btn btn-success btn-block" role="button" name="copy">前月コピー</button></td>
              <?php } ?>
      ...
      <div class="row">
          ...
          <div class="col-6">
              <div class="d-inline-block">勤務体系</div>
              <!-- シフト -->
              ...
              <?php if($_REQUEST['kensyu'] == '1'){ $ken_checked = ' checked = "checked" '; } ?>
              <label>
                　<input type="checkbox" id="kensyu" name="kensyu" value="1" <?php echo($ken_checked); ?> <?php print($disabled); ?>> <span style="background-color:#CCFF99;">研</span>　
              </label>
          </div>
      ...
      ?>
      <?php if ((isset($_REQUEST['search']) || $staff2->oup_m_staff_auth[0] == "2") && $genba_id != "") { ?>
      <div class="row">
        <div class="col-xs-12 col-md-12">
<?php /* ?>
...
<?php */ ?>
          ...
          <table border="1" class="sheet m-auto" style="border-collapse: collapse; table-layout: fixed;">
            ...
            <tbody>
              ...
              <!-- ajax -->
              <!-- <tr class="text-center tr<?php echo $i ?>"> -->

              <!-- ajax -->
              <!-- <input type="hidden" class="wk_taiin<?php echo $i ?>" value="<?php echo count($wk->oup_t_wk_no).",".$wk->oup_t_wk_no[$i].",".$wk->oup_t_wk_taiin_id[$i] ?>"> -->
                ...
                <?php } else { ?>
                    <?php //if ($genba_flg != true) { ?>
                        ...
                        <?php } else { ?>
                        <td>
                            <a name="#<?php print($wk->oup_t_wk_no[$i]); ?>">
                            <!--<a href="kinmuyotei.php?search=&up=&wk_no=<?php print($wk->oup_t_wk_no[$i]); ?>&genba_id=<?php print($genba_id); ?>&nengetu=<?php print($nengetu); ?>" class="btn btn-success btn-block" role="button" aria-pressed="true">↑</a>-->
                            <a href="#<?php print($wk->oup_t_wk_no[$i]); ?>" onclick="up(<?php print($wk->oup_t_wk_no[$i]); ?>)" class="btn btn-success btn-block" role="button" aria-pressed="true">↑</a>
                        </td>
                        <?php } ?>
                        ...
                        <?php } else { ?>
                        <td>
                            <!--<a href="kinmuyotei.php?search=&down=&wk_no=<?php print($wk->oup_t_wk_no[$i]); ?>&genba_id=<?php print($genba_id); ?>&nengetu=<?php print($nengetu); ?>" class="btn btn-success btn-block" role="button" aria-pressed="true">↓</a>-->
                            <a href="#<?php print($wk->oup_t_wk_no[$i]); ?>" onclick="down(<?php print($wk->oup_t_wk_no[$i]); ?>)" class="btn btn-success btn-block" role="button" aria-pressed="true">↓</a>
                        </td>
                        <?php } ?>
                    <?php //} else { ?>
                ...
                <?php /*<!-- ajax -->*/ ?>
                <?php //print ($align_center); ?>

                <td class="td" <?php print ($align_center); ?> style="background-color:<?php echo $color[$wk->oup_t_wk_taiin_id[$i]][substr($nengetu, 0, 4)."-".substr($nengetu, 4, 2)."-".$day]; ?>">
                  <?php } ?>

                  <!-- ajax -->
                  <!-- <?php echo "<a href=\"#".$wk->oup_t_wk_no[$i]."\"></a>" ?> -->
                ...
                <!-- ajax -->
                <input type="hidden" name="detail" value="<?php echo $wk->oup_t_wk_taiin_id[$i].','.$j ?>">
      ...
      <input type="hidden" name="conf_flg" id="conf_flg" value="<?php print($conf_flg); ?>">
