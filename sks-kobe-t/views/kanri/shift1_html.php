...
<script>
function confirm1(no,genba) {
  if (!confirm(genba +'の項目を削除してもよろしいですか?')) return false;
...
<?php for ($i=0;$i<count($shift->oup_m_shift_no);$i++) {
            ...
            <tr align="center" <?php echo $shift->oup_m_shift_deleteday[$i] != "" ? "bgcolor='#d5d5d5'" : "" ; ?>>
              <td>
              ...
              <td align="right"><?php print($shift->oup_m_shift_deleteday[$i]); ?></td>

              <td><a href="javascript:void(0);" onClick="confirm1(<?php print($shift->oup_m_shift_no[$i]); ?>,'<?php print($genba_m[$shift->oup_m_shift_genba_id[$i]]); ?>');">削</a></td>
            </tr>
            <?php
            } ?>
          </table>
