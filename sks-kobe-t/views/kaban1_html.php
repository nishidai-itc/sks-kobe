...
//端末情報の取得
var device = '<?php echo $common->device ?>';
...
$(function() {
  ...
  $('input[id="tujo_checked"]').change(function() {
    if ($(this).prop('checked')) {
      ...
      if (device != 'mobile' && device != 'tablet') {
        $(this).parent().parent().children('td').find('.kaban-time1').val(planJobanTime.substr(0,2));
        $(this).parent().parent().children('td').find('.kaban-time2').val(planJobanTime.substr(3,2));
        $(this).parent().parent().children('td').find('.kaban-time').val(planJobanTime.substr(0,2)+":"+planJobanTime.substr(3,2));
        
        $(this).parent().parent().children('td').find('.kaban-time1').prop('readonly', true);
        $(this).parent().parent().children('td').find('.kaban-time2').prop('readonly', true);
      //パソコン以外での処理
      } else {
      ...
      //パソコンでの処理
      if (device != 'mobile' && device != 'tablet') {
        $(this).parent().parent().children('td').find('.kaban-time1').prop('readonly', false);
        $(this).parent().parent().children('td').find('.kaban-time2').prop('readonly', false);
        $(this).parent().parent().children('td').find('.kaban-time1').val('');
        $(this).parent().parent().children('td').find('.kaban-time2').val('');
        $(this).parent().parent().children('td').find('.kaban-time').val('');
      //パソコン以外での処理
      } else {
  ...
  //パソコンでの処理
  if (device != 'mobile' && device != 'tablet') {
    $('input[name="kaban-time1"]').change(function(){
    var kt1 = '';
        if ($(this).val() == '') {
            kt1 = ''
        } else {
            kt1 = ('00'+$(this).val()).slice(-2)
        }
        if (kt1 != '' && $(this).parent().children('.kaban-time2').val() != '') {
            $(this).parent().parent().parent().parent().children('td').find('.kaban-time').val(kt1+":"+('00'+$(this).parent().children('.kaban-time2').val()).slice(-2))
        } else {
            $(this).parent().parent().parent().parent().children('td').find('.kaban-time').val('')
        }
    });
    $('input[name="kaban-time2"]').change(function(){
    var kt2 = '';
        if ($(this).val() == '') {
            kt2 = ''
        } else {
            kt2 = ('00'+$(this).val()).slice(-2)
        }
        if ($(this).parent().children('.kaban-time1').val() && kt2 != '') {
            $(this).parent().parent().parent().parent().children('td').find('.kaban-time').val(('00'+$(this).parent().children('.kaban-time1').val()).slice(-2)+":"+kt2)
        } else {
            $(this).parent().parent().parent().parent().children('td').find('.kaban-time').val('')
        }
    });
...
<body>
  <div class="container">
    ...
    <form method="POST" action="kaban1.php">
          ...
          <table class="table table-sm table-bordered">
            ...
            <tbody>
              <tr>
                ...
                <td class="table-info text-center font-weight-bold" colspan="4">下番報告（リーダー用）</td>
              </tr>
          ...
          <table class="table table-sm table-bordered text-nowrap">
            ...
            <thead>
              <tr class="text-center">
                ...
                <td class="table-warning">氏名<br>(勤務,補足)</td>
            ...
            <tbody class="table-body-font">
              ...
              <?php for ($i=0; $i<count($work_mem2->oup_t_wk_taiin_id); $i++) { 
                    //if ($shift->kbn[$work_mem2->oup_t_wk_plan_kbn[$i]] != "年" && $work_mem2->oup_t_wk_joban_kbn[$i] != "4" && $work_mem2->oup_t_wk_joban_kbn[$i] != "5") {?>
              <tr class="text-center">
                <!-- 現場 -->
                <td>
                  <?php print($genba_name[$work_mem2->oup_t_wk_genba_id[$i]]); ?>
                  
                </td>
                <!-- 氏名 -->
                <td>
                  <?php print($staffs[$work_mem2->oup_t_wk_taiin_id[$i]]); ?>
                  <br><?php print(substr($work_mem2->oup_t_wk_plan_date[$i],5,2)."/".substr($work_mem2->oup_t_wk_plan_date[$i],8,2)); ?>
                  (<?php echo $work_mem2->oup_t_wk_plan_hosoku[$i] != "" ? $shift->kbn[$work_mem2->oup_t_wk_plan_kbn[$i]].$work_mem2->oup_t_wk_plan_hosoku[$i] : $shift->kbn[$work_mem2->oup_t_wk_plan_kbn[$i]]; ?>)
                ...
                <!-- 下番時刻 -->
                <td class="align-middle" <?php echo $common->device == "pc" ? "width=130" : "" ; ?>>
                
                <!-- PC以外 -->
                <?php if ($common->device == "mobile" || $common->device == "tablet") { ?>
                ...
                <?php } else { ?>
                  <div class="input-form-size">
                    <div class="input-group input-group-sm">
                      <input type="number" class="form-control input-form-right text-center kaban-time1" name="kaban-time1" value="" min="0" max="23">
                      <div class="input-group-prepend input-group-colon">
                        <span class="input-group-text">:</span>
                      </div>
                      <input type="number" class="form-control input-form-left text-center kaban-time2" name="kaban-time2" value="" min="0" max="59">
                    </div>
                  </div>
                  <input type="hidden" class="kaban-time" name="kaban_time[<?php echo $i; ?>]" value="">
                <?php } ?>
            ...
            <?php } ?>
              ...
              <!-- 当日分 -->
              <?php if ($work_mem->oup_t_wk_taiin_id) { ?>
              <?php for ($i=0;$i<count($work_mem->oup_t_wk_taiin_id);$i++) {
                    //if ($shift->kbn[$work_mem->oup_t_wk_plan_kbn[$i]] != "年" && $work_mem->oup_t_wk_joban_kbn[$i] != "4" && $work_mem->oup_t_wk_joban_kbn[$i] != "5") { ?>
              <tr class="text-center">
                <!-- 現場 -->
                <td>
                ...
                <td>
                  ...
                  <br>
                  <?php print(substr($work_mem->oup_t_wk_plan_date[$i],5,2)."/".substr($work_mem->oup_t_wk_plan_date[$i],8,2)); ?>
                  (<?php echo $work_mem->oup_t_wk_plan_hosoku[$i] != "" ? $shift->kbn[$work_mem->oup_t_wk_plan_kbn[$i]].$work_mem->oup_t_wk_plan_hosoku[$i] : $shift->kbn[$work_mem->oup_t_wk_plan_kbn[$i]]; ?>)
                  <input type="hidden" name="kaban_dakoku[<?php echo $work_mem2->oup_t_wk_taiin_id ? $i + count($work_mem2->oup_t_wk_taiin_id) : $i; ?>]" value="<?php print($work_mem->oup_t_wk_kaban_dakoku_time[$i]); ?>">
                </td>
                <!-- 上番下番 -->
                <td>
                ...
                <td class="align-middle">
                  ...
                  <input type="checkbox" name="tujo_checked[<?php echo $work_mem2->oup_t_wk_taiin_id ? $i + count($work_mem2->oup_t_wk_taiin_id) : $i; ?>]" id="tujo_checked" 
                    value="<?php print($work_mem->oup_t_wk_plan_kaban_time[$i]); ?>">
                  	<?php /* 添字番号が当日と重なるので添字に当日の件数をプラス */ ?>
                  <input type="hidden" name="plan_kaban[<?php echo $work_mem2->oup_t_wk_taiin_id ? $i + count($work_mem2->oup_t_wk_taiin_id) : $i; ?>]" value="<?php print($work_mem->oup_t_wk_plan_kaban_time[$i]); ?>">
                </td>
                <!-- 下番時刻 -->
                <td class="align-middle" <?php echo $common->device == "pc" ? "width=130" : "" ; ?>>
                  <!-- <?php
                ...
                <?php if ($common->device == "mobile" || $common->device == "tablet") { ?>
                  <input type="time" class="form-control form-control-sm kaban-time"
                    name="kaban_time[<?php echo $work_mem2->oup_t_wk_taiin_id ? $i + count($work_mem2->oup_t_wk_taiin_id) : $i; ?>]"
                ...
                <?php } else { ?>
                  ...
                  <input type="hidden" class="kaban-time" name="kaban_time[<?php echo $work_mem2->oup_t_wk_taiin_id ? $i + count($work_mem2->oup_t_wk_taiin_id) : $i; ?>]" value="">
                <?php }?>
                ...
                <td class="align-middle">
                  ...
                  <select class="form-control form-control-sm"
                    name="daytime_over_time[<?php echo $work_mem2->oup_t_wk_taiin_id ? $i + count($work_mem2->oup_t_wk_taiin_id) : $i; ?>]">
                    <?php foreach ($select_daytime as $time => $minute) {?>
                    <option value="<?php echo $time; ?>"
                ...
                <td class="align-middle">
                  ...
                  <select class="form-control form-control-sm"
                    name="rest_over_time[<?php echo $work_mem2->oup_t_wk_taiin_id ? $i + count($work_mem2->oup_t_wk_taiin_id) : $i; ?>]">
                    <?php foreach ($select_daytime as $time => $minute) {?>
                    <option value="<?php echo $time; ?>"
                ...
                <td class="align-middle">
                  ...
                  <select class="form-control form-control-sm"
                    name="midnight_over_time[<?php echo $work_mem2->oup_t_wk_taiin_id ? $i + count($work_mem2->oup_t_wk_taiin_id) : $i; ?>]">
                    <?php foreach ($select_daytime as $time => $minute) {?>
                    <option value="<?php echo $time; ?>"
...
<?php */ ?>
                  ...
                  <select class="form-control form-control-sm" name="post_teate[<?php echo $work_mem2->oup_t_wk_taiin_id ? $i + count($work_mem2->oup_t_wk_taiin_id) : $i; ?>]">
                    <option value=""></option>
                    <?php if ($postteate->oup_m_post_teate_no) { ?>
                    <?php for ($j=0;$j<count($postteate->oup_m_post_teate_no);$j++) { ?>
                    <option value="<?php echo $postteate->oup_m_post_teate_post_cost[$j]; ?>"
                ...
                <td class="align-middle">
                  <input type="number" class="form-control form-control-sm"
                    name="kotuhi[<?php echo $work_mem2->oup_t_wk_taiin_id ? $i + count($work_mem2->oup_t_wk_taiin_id) : $i; ?>]"
                    value="<?php print($work_mem->oup_t_wk_kotuhi[$i]); ?>">
                </td>
              </tr>

              <INPUT type="hidden" name="wk_no[<?php echo $work_mem2->oup_t_wk_taiin_id ? $i + count($work_mem2->oup_t_wk_taiin_id) : $i; ?>]"
                value="<?php echo $work_mem->oup_t_wk_detail_no[$i]; ?>">
              <INPUT type="hidden" name="plan_time[<?php echo $work_mem2->oup_t_wk_taiin_id ? $i + count($work_mem2->oup_t_wk_taiin_id) : $i; ?>]"
                id="plan_time<?php echo $work_mem2->oup_t_wk_taiin_id ? $i + count($work_mem2->oup_t_wk_taiin_id) : $i; ?>"
                value="<?php print($work_mem->oup_t_wk_plan_kaban_time[$i]); ?>">
