...
<head>
  ...
  <!-- <link href="./../clockpicker/src/jquery-clockpicker.min.css" rel="stylesheet">
  <script src="./../clockpicker/src/jquery-clockpicker.min.js"></script> -->
  <!-- <link href="./../clockpicker/src/bootstrap-clockpicker.min.css" rel="stylesheet">
  <script src="./../clockpicker/src/bootstrap-clockpicker.min.js"></script> -->
...
<body>
  ...
  $(function() {
    ...
    $('input[id="tujo_checked"]').change(function() {
      if ($(this).prop('checked')) {
        ...
        $(this).parent().parent().children('td').find('.joban-time').val('');
      }
    });
    // 全選択
    ...
    $('input[id="tujo_checked"]').change(function() {
      if ($(this).prop('checked')) {
        ...
        if (device != 'mobile' && device != 'tablet') {
          $(this).parent().parent().children('td').find('.joban-time1').val(planJobanTime.substr(0,2));
          $(this).parent().parent().children('td').find('.joban-time2').val(planJobanTime.substr(3,2));
          $(this).parent().parent().children('td').find('.joban-time').val(planJobanTime.substr(0,2)+":"+planJobanTime.substr(3,2));
          
          $(this).parent().parent().children('td').find('.joban-time1').prop('readonly', true);
          $(this).parent().parent().children('td').find('.joban-time2').prop('readonly', true);
        //パソコン以外での処理
        } else {
          $(this).parent().parent().children('td').find('.joban-time').val(planJobanTime);
        ...
        //パソコンでの処理
        if (device != 'mobile' && device != 'tablet') {
          $(this).parent().parent().children('td').find('.joban-time1').prop('readonly', false);
          $(this).parent().parent().children('td').find('.joban-time2').prop('readonly', false);
          $(this).parent().parent().children('td').find('.joban-time1').val('');
          $(this).parent().parent().children('td').find('.joban-time2').val('');
          $(this).parent().parent().children('td').find('.joban-time').val('');
        //パソコン以外での処理
        } else {
    ...
    //パソコンでの処理
    if (device != 'mobile' && device != 'tablet') {
      $('input[name="joban-time1"]').change(function(){
      var jt1 = '';
          if ($(this).val() == '') {
              jt1 = ''
          } else {
              jt1 = ('00'+$(this).val()).slice(-2)
          }
          if (jt1 != '' && $(this).parent().children('.joban-time2').val() != '') {
              $(this).parent().parent().parent().parent().children('td').find('.joban-time').val(jt1+":"+('00'+$(this).parent().children('.joban-time2').val()).slice(-2))
          } else {
              $(this).parent().parent().parent().parent().children('td').find('.joban-time').val('')
          }
      });
      $('input[name="joban-time2"]').change(function(){
      var jt2 = '';
          if ($(this).val() == '') {
              jt2 = ''
          } else {
              jt2 = ('00'+$(this).val()).slice(-2)
          }
          if ($(this).parent().children('.joban-time1').val() && jt2 != '') {
              $(this).parent().parent().parent().parent().children('td').find('.joban-time').val(('00'+$(this).parent().children('.joban-time1').val()).slice(-2)+":"+jt2)
          } else {
              $(this).parent().parent().parent().parent().children('td').find('.joban-time').val('')
          }
      });
  ...
  <div class="container">
    ...
    <form method="POST" action="joban1.php">
      <div class="row my-1">
        <div class="<?php echo $col1 ?> text-center p-0">
          隊員
        </div>
        <div class="<?php echo $col1 ?> p-0">
        ...
        <div class="<?php echo $common->device == "tablet" ? "col-3" : $col2; ?>">
          <select class="form-control" name="add_staff" required>
            <option></option>
            <?php
            if ($search_staff->oup_m_staff_id) {
            ...
            <option class="staff" name="<?php print(mb_convert_kana(mb_substr($search_staff->oup_m_staff_kana[$i],0,1),"K")); ?>" 
            value="<?php print($search_staff->oup_m_staff_id[$i]); ?>">
      ...
      <?php } ?>
        ...
        <div class="<?php echo $col2 ?>">
          <select class="form-control" name="shift" required>
            <option></option>
            <?php
            if ($shift2->oup_m_shift_no) {
            for ($i=0;$i<count($shift2->oup_m_shift_no);$i++) { ?>
            <option value="<?php print($shift2->oup_m_shift_no[$i]); ?>">
              <?php print($shift_name[$shift2->oup_m_shift_no[$i]]); ?></option>
            <?php
            }
            } ?>
          </select>
        ...
        <div class="<?php echo $common->device == "tablet" ? "col-2" : $col1; ?>">
...
//              if ($shift->kbn2[$work_mem->oup_t_wk_plan_kbn[$i]] != "年休") {?>
            ...
            <tr class="text-center">
              ...
              <div class="input-group clockpicker">
                <input type="text" class="form-control joban-time" value="<?php print($work_mem->oup_t_wk_joban_time[$i]); ?>" name="joban_time[<?php echo $i; ?>]" readonly>
              </div>
              <?php /* ?>
              <!-- PC以外 -->
              <?php if ($common->device == "mobile" || $common->device == "tablet") { ?>
                  <input type="time" class="form-control form-control-sm joban-time" name="joban_time[<?php echo $i; ?>]" value="<?php print($work_mem->oup_t_wk_joban_time[$i]); ?>">
              <!-- PC -->
              <?php } else { ?>
                  <div class="input-form-size">
                    <div class="input-group input-group-sm">
                      <input type="number" class="form-control input-form-right text-center joban-time1" name="joban-time1" value="" min="0" max="23">
                      <div class="input-group-prepend input-group-colon">
                        <span class="input-group-text">:</span>
                      </div>
                      <input type="number" class="form-control input-form-left text-center joban-time2" name="joban-time2" value="" min="0" max="59">
                    </div>
                  </div>
                  <input type="hidden" class="joban-time" name="joban_time[<?php echo $i; ?>]" value="">
              <?php } ?>
              <?php */ ?>
