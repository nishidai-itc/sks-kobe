...
<?php
    ...
    */
    ...
    // 作業実施テーブル 取得 に必要な情報をセット
    $todaywork->inp_t_wk_taiin_id  = $_SESSION["staff_id"];
    $todaywork->inp_t_wk_plan_date = date('Ymd');
    $todaywork->inp_t_wk_del_flg   = "0";

    // 作業実施テーブル 取得
    $todaywork->getWkdetail();
    // 作業実施データが取得できた場合
    if (count($todaywork->oup_t_wk_detail_no) != 0) {
        // 更新フラグをtrueにする
        $todayworkflg = true;
    }
    */
...
<?php
    ...
    if ($common->judgephone) {
        // HTML表示
        require_once('../views/menu_html.php');
    // キャリア判定（フィーチャーフォン）
    } else {
        // HTML表示
