...
//        $new_pw2 = $_POST["new_pw2"];
    ...
    if (isset($_POST["joban_dakoku"])) {
        ...
        $work_yesterday       = new Wkdetail;     // 作業クラス
        $work_yesterday->inp_t_wk_taiin_id = $_SESSION["staff_id"];
        $work_yesterday->inp_t_wk_plan_date = date('Ymd', strtotime('-1 day'));
        $work_yesterday->inp_t_wk_joban_kbn = "1";
        $work_yesterday->inp_t_wk_kaban_kbn_not_in = "1";
        $work_yesterday->getWkdetail();
        // var_dump($work_yesterday->oup_t_wk_detail_no);
        // exit;
        if ($work_yesterday->oup_t_wk_detail_no) {
            $alert = "<script type='text/javascript'>alert('前日の勤務を先に下番して下さい。');</script>";
            echo $alert;
        } else {
            ...
            $work->inp_t_wk_joban_kbns    = "4,5";
            $work->inp_t_wk_del_flg     = "0";

            $work->inp_order     = "order by t_wk_plan_joban_time";
            ...
            if ($work->oup_t_wk_detail_no[0] == null) {
                ...
                $work_mem->inp_t_wk_created = date("Y-m-d H:i:s");
                $work_mem->inp_t_wk_created_id = $_SESSION["staff_id"];
                $work_mem->insertWkdetail();

                $alert = "<script type='text/javascript'>alert('上番打刻しました。');</script>";
                echo $alert;

            } elseif ($work->oup_t_wk_detail_no) {
                for ($i=0;$i<count($work->oup_t_wk_detail_no);$i++) {

                    if ($work->oup_t_wk_joban_kbn[$i]=="" || !$work->oup_t_wk_joban_kbn[$i]) {
                        ...
                        $work_mem->inp_t_wk_modified = date("Y-m-d H:i:s");
                        $work_mem->inp_t_wk_modified_id = $_SESSION["staff_id"];
                        ...
                        */
                        ...
                        if (count($work->oup_t_wk_detail_no) == 1) {
                            $alert = "<script type='text/javascript'>alert('既に上番打刻されています。');</script>";
                            echo $alert;
                            break;
                        } else {
                            if ($work->oup_t_wk_kaban_kbn[$i]=="" || !$work->oup_t_wk_kaban_kbn[$i]) {
                                if ($i == (count($work->oup_t_wk_detail_no) - 1)) {
                                    $alert = "<script type='text/javascript'>alert('既に上番打刻されています。');</script>";
                                    echo $alert;
                                    break;
                                } else {
                                    $alert = "<script type='text/javascript'>alert('先に下番打刻して下さい。');</script>";
                                    echo $alert;
                                    break;
                                }
                            } else {
    ...
    if (isset($_POST["kaban_dakoku"])) {
        ...
        // 昨日の実績か今日の実績か判定
        if ($work2->oup_t_wk_detail_no && is_null($work2->oup_t_wk_kaban_time[0])) {
...
//                        $work_mem->inp_t_wk_kaban_time = date("H:i:s");
                        ...
                        if ($work->oup_t_wk_inp_kbn[$i] != "1") {
                            ...
                            $work2->inp_plan_kaban_time = $work->oup_t_wk_plan_kaban_time[$i];
                            $work2->inp_joban_time = $work->oup_t_wk_joban_time[$i];
                            // $work2->inp_kaban_time = $work->oup_t_wk_plan_kaban_time[$i];
                            $work2->inp_kaban_time = $work->oup_t_wk_kaban_time[$i];
                            $work2->getcalckintime();
                        ...
                        if (count($work->oup_t_wk_detail_no) == 1 || $i == (count($work->oup_t_wk_detail_no) - 1)) {
                            $errmsg = "既に下番打刻されています。";
                            break;
                        }
                    }
