...
<?php
    ...
    if (isset($_REQUEST['kyuyo_kbn'])) {
        $kyuyo_kbn = $_REQUEST['kyuyo_kbn'];
    }
    ...
    if ($flg == "1") {
        $startday = $nengetu."01";
        $endday = $nengetu.substr(date('Ymd',mktime(0,0,0,substr($nengetu,4,2)+1,0,substr($nengetu,0,4))),6,2);
    }
...
//        $shift2_col[$shift->oup_m_shift_genba_id[$i]][$shift->oup_m_shift_plan_kbn[$i]] = $shift->oup_m_shift_color[$i];
    ...
    if (isset($_REQUEST["bundle"])) {
        //Gﾁｪｯｸ済登録
        if (isset($_POST["check_teate"]) && $_POST["gchk"] != "") {
    ...
    fclose($fp);
        ...
        //Sﾁｪｯｸ済登録
        if (isset($_POST["check_teate"]) && $_POST["schk"] != "") {
                $check_teate = $_POST["check_teate"];
                $chk = $_POST["schk"];
                // 登録
                foreach ($check_teate as $wkdetail_no) {
    ...
    fclose($fp);
        ...
        if (isset($_POST["select_teate"]) && $_POST["select_teate"] != "") {
            if (isset($_POST["check_teate"]) && isset($_POST["input_teate"]) && $_POST["input_teate"] != "") {
    ...
    if ($staff2->oup_m_staff_auth[0]=="4" || $staff2->oup_m_staff_auth[0]=="2") {
        if ($staff2->oup_m_staff_auth[0]=="2") {
            $oya_genba = new Genba;
            $oya_genba->inp_m_genba_oya_id = $staff2->oup_m_staff_genba_id[0];
            $oya_genba->getGenba();
            
            $genba4->inp_m_genba_id2 = "'".$staff2->oup_m_staff_genba_id[0]."'";
            if ($oya_genba->oup_m_genba_id) {
            for ($i=0;$i<count($oya_genba->oup_m_genba_id);$i++) {
                $genba4->inp_m_genba_id2 = $genba4->inp_m_genba_id2.",'".$oya_genba->oup_m_genba_id[$i]."'";
            }
            }
        } else {
    ...
    if (isset($_REQUEST["genba_id"])) {
        ...
        if ($_SESSION["gid"] != "") {
            ...
            // if ($staff2->oup_m_staff_auth[0]=="2") {
            if ($staff2->oup_m_staff_auth[0]=="2" && $flg != 1) {
    ...
    if (!(($staff2->oup_m_staff_auth[0]=="1") || ($staff2->oup_m_staff_auth[0]=="3"))) {
        ...
        if ($flg != 1 && $staff2->oup_m_staff_auth[0]=="4") {
            $genba_id[0] = $staff2->oup_m_staff_genba_id[0];
        }
    }
...
//    $genba2->inp_m_genba_oya_id = $genba_id[0];
    ...
    // 予定の取得
    //勤務照会用(隊員と管理メニューで分ける)
    if ($flg == 1 && $_SESSION["menu_flg"] != "kanri" || ($flg == 1 && $_SESSION["menu_flg"] == "kanri" && $staff_id != "")) {
        ...
        // $last_day = substr(date('Ymd', strtotime('last day of ' . substr($nengetu,0,4).'-'.substr($nengetu,4,2))),6,2)       ←〇
        //mktime(時,分,秒,月,日,年)  日を0,月をdate('m')+1,年をdate('Y')
        $last_day = substr(date('Ymd',mktime(0,0,0,substr($nengetu,4,2)+1,0,substr($nengetu,0,4))),6,2);
        $wkdetail->inp_t_wk_plan_end_date = $nengetu.$last_day;
        $wkdetail->inp_order = "ORDER BY t_wk_detail.t_wk_plan_date,t_wk_detail.t_wk_plan_joban_time ";
        //$wkdetail->getWkdetail();
    }
//    if (!isset($_REQUEST["prev2"]) && !isset($_REQUEST["next2"])) {
    if ($flg == 1 && $_SESSION["menu_flg"] == "kanri" && $staff_id == "") {
