...
if (isset($_POST["detail_no"])) {
    ...
    //連勤残手入力
    if ($_REQUEST["renzan_kbn"] == 1) {
        //入力区分
        $renzan_kbn = 1;
        
        //連勤残
        //if ($_POST["renzan"][0] == "" && $_POST["renzan"][1] == "") {
        //    $renzan = "";
        //} elseif($_POST["renzan"][0] != 0 && $_POST["renzan"][1] != 0) {
        //    $renzan = $_POST["renzan"][0].".".sprintf("%02d",$_POST["renzan"][1]);
        //}
        if ($_POST["renzan"][0] == "" || $_POST["renzan"][0] == 0) {
            //$renzan0 = "00";
            $renzan0 = "";
        } else {
            //$renzan0 = sprintf("%02d",$_POST["renzan"][0]);
            $renzan0 = $_POST["renzan"][0]*60;
        }
        if ($_POST["renzan"][1] == "" || $_POST["renzan"][1] == 0) {
            //$renzan1 = "00";
            $renzan1 = "";
        } else {
            //$renzan1 = sprintf("%02d",$_POST["renzan"][1]);
            $renzan1 = $_POST["renzan"][1];
        }
        if ($renzan0 > 0 || $renzan1 > 0) {
            if ($renzan1%15 == 0) {
                //$renzan = $renzan0.".".$renzan1;
                $renzan = $renzan0+$renzan1;
                //var_dump($renzan/60);
                //exit;
                //if ($renzan >= 60) {
                //    $renzan = ($renzan/60).".".sprintf("%02d",$renzan/15*25);
                //} else {
                //    $renzan = "0.".sprintf("%02d",$renzan/15*25);
                //}
                $renzan = strval(sprintf("%.2f",$renzan/60));
//                $renzan_kbn = 1;
            }
        } elseif ($renzan0 == 0 && $renzan1 == 0) {
//            $renzan_kbn = "";
            $renzan = "";
        }
        //if ($_POST["renzan"][0] != "" && $_POST["renzan"][1] != "") {
        //    $renzan = sprintf("%02d",$_POST["renzan"][0]).".".sprintf("%02d",$_POST["renzan"][1]);
        //    $renzan_kbn = 1;
        //} elseif ($_POST["renzan"][0] < 1 && $_POST["renzan"][1] < 1) {
        //    $renzan = "";
        //}
    } else {
        $renzan_kbn = "";
    ...
    //}
    // 時間外勤務 深夜残業
    if(($_POST["midnight_over_time"][0] == "") && ($_POST["midnight_over_time"][1] == "")) {
        $midnight_over_time = "";
    } else {
    	$midnight_over_time = $utils->formatHourMinute($_POST["midnight_over_time"][0], $_POST["midnight_over_time"][1]);
	}
    // 手当て ポスト
    ...
    // 上番区分
    if ($_POST["joban_kbn"] == 1) {
        $joban_kbn = $_POST["joban_kbn"];
    } elseif ($_POST["joban_kbn1"] == 4) {
        $joban_kbn = $_POST["joban_kbn1"];
    } elseif ($_POST["joban_kbn2"] == 5) {
        $joban_kbn = $_POST["joban_kbn2"];
    } else {
        $joban_kbn = "";
    }
...
fclose($fp);
    ...
    if ($_REQUEST["renzan_kbn"] == 1) {
        $wkdetail->inp_t_wk_renzan_kbn = $renzan_kbn;
        $wkdetail->inp_t_wk_renzan = $renzan;
    } else {
        $wkdetail->inp_t_wk_renzan_kbn = $renzan_kbn;
        //$wkdetail->inp_t_wk_renzan = $renzan;
    }
    ...
    //勤務体系変更時予定表に隊員がなければ予定を追加する
    if ($genba_id != "9999") {
        $wk = new Wk;
        $wk->inp_t_wk_nengetu = substr($_POST["plan_date"], 0, 4).substr($_POST["plan_date"], 5, 2);
        $wk->inp_t_wk_taiin_id = $_POST["staff"];
        $wk->inp_t_wk_genba_id = $genba_id;
        $wk->getWk();
        if (!$wk->oup_t_wk_no) {
        // if (count($wk->oup_t_wk_no) < 1) {
            $wk2 = new Wk;
            $wk2->inp_t_wk_nengetu = substr($_POST["plan_date"], 0, 4).substr($_POST["plan_date"], 5, 2);
            $wk2->inp_t_wk_genba_id = $genba_id;
            $wk2->inp_order = "ORDER BY t_wk_order DESC limit 1";
            $wk2->getWk();
            
            if ($wk2->oup_t_wk_order) {
                $wk->inp_t_wk_order = ($wk2->oup_t_wk_order[0] + 10);
            } else {
                $wk->inp_t_wk_order = 10;
            }
            $wk->inp_t_wk_created = date("Y-m-d H:i:s");
            $wk->inp_t_wk_created_id = $_SESSION["staff_id"];
            $wk->insertWk();
        }
    }
    ...
    //年休、欠勤の場合実績を削除
    if ($joban_kbn == 4 || $joban_kbn == 5) {
        $wkdetail->inp_t_wk_joban_time = ""; // 実績 上番
        $wkdetail->inp_t_wk_kaban_time = ""; // 実績 下番
    } else {
