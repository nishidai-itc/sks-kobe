...
//        ;
    ...
    // 検索月の祝日を取得
    if ($flg == "3") {
        $holiday->inp_holiday_start_date = date("Ymd",strtotime($nengetu."01"." -1 day"));
        $holiday->inp_holiday_end_date = $nengetu.substr(date('Ymd',mktime(0,0,0,substr($nengetu,4,2)+1,0,substr($nengetu,0,4))),6,2);
    } else {
    ...
    for ($i=0;$i<count($holiday->oup_date);$i++) {
        if ($i==0) {
            ...
            $wk_holiday = substr($holiday->oup_date[$i],0,4).substr($holiday->oup_date[$i],5,2).substr($holiday->oup_date[$i],8,2);
        } else {
            // $wk_holiday = $wk_holiday.",".substr($holiday->oup_date[$i],8,2);
            $wk_holiday = $wk_holiday.",".substr($holiday->oup_date[$i],0,4).substr($holiday->oup_date[$i],5,2).substr($holiday->oup_date[$i],8,2);
        }
    }
    }
    ...
    // 入力年月最終日
    $toLastday = substr(date('Ymd', strtotime($nengetu.'01'.'last day of this month')),6,2);
...
//         }
    ...
    //実績表画面の場合検索と同時に連勤残計算、給与連携処理（総務）画面の場合集計と同時に連勤残計算
    if ($act == "0" || $act == "2") {
    //// 連勤残計算
    //if ($act == "1") {
...
//    if (($act == "1") || ($act == "2")) {
    if ($act == "2") {
        ...
        $detail->getWkdetail();
        if (!$detail->oup_t_wk_detail_no) {
            echo json_encode($detail->oup_t_wk_detail_no);
            exit;
        }
        ...
        for ($i=0;$i<count($wk->oup_t_wk_no);$i++) {
            ...
            for ($j=1;$j<=31;$j++) {
                ...
                if ($kbn2[$wk->oup_t_wk_taiin_id[$i]][$nengetuwk][$wk->oup_t_wk_genba_id[$i]] != "") {
                    ...
                    // ポスト手当て
                    $kyuyo[$taiin_id]["s12"] = (int)$kyuyo[$taiin_id]["s12"] + (int)$post[$taiin_id][$nengetuwk][$wk->oup_t_wk_genba_id[$i]];
                    if (($post[$taiin_id][$nengetuwk][$wk->oup_t_wk_genba_id[$i]]=="")||(($post[$taiin_id][$nengetuwk][$wk->oup_t_wk_genba_id[$i]]==0))) {
                    } else {
                        $kyuyo[$taiin_id]["k12"] = (int)$kyuyo[$taiin_id]["k12"]+6;
                    }

                    // 通勤費
                    $kyuyo[$taiin_id]["s13"] = (int)$kyuyo[$taiin_id]["s13"] + (int)$tukin[$taiin_id][$nengetuwk][$wk->oup_t_wk_genba_id[$i]];

                    $kyuyo[$taiin_id]["s15"] = (int)$kyuyo[$taiin_id]["s15"] + (int)$shogatu[$taiin_id][$nengetuwk][$wk->oup_t_wk_genba_id[$i]];
                    $kyuyo[$taiin_id]["s16"] = (int)$kyuyo[$taiin_id]["s16"] + (int)$kaki[$taiin_id][$nengetuwk][$wk->oup_t_wk_genba_id[$i]];
                    $kyuyo[$taiin_id]["s17"] = (int)$kyuyo[$taiin_id]["s17"] + (int)$teate1[$taiin_id][$nengetuwk][$wk->oup_t_wk_genba_id[$i]];
                    $kyuyo[$taiin_id]["s18"] = (int)$kyuyo[$taiin_id]["s18"] + (int)$teate2[$taiin_id][$nengetuwk][$wk->oup_t_wk_genba_id[$i]];
                    $kyuyo[$taiin_id]["s19"] = (int)$kyuyo[$taiin_id]["s19"] + (int)$teate3[$taiin_id][$nengetuwk][$wk->oup_t_wk_genba_id[$i]];

                    // 年休
                    if (($jbnkbn[$taiin_id][$nengetuwk][$wk->oup_t_wk_genba_id[$i]] == "4") || ($kbn2[$wk->oup_t_wk_taiin_id[$i]][$nengetuwk][$wk->oup_t_wk_genba_id[$i]] == "4")) {

                        $kyuyo[$taiin_id]["k13"] = (int)$kyuyo[$taiin_id]["k13"] + 1;
                        ...
                        if ($joban[$wk->oup_t_wk_taiin_id[$i]][$nengetuwk][$wk->oup_t_wk_genba_id[$i]] != "" && $kaban[$wk->oup_t_wk_taiin_id[$i]][$nengetuwk][$wk->oup_t_wk_genba_id[$i]] != "") {
                            ...
                            if ($jikyuflg[$taiin_id][$nengetuwk][$wk->oup_t_wk_genba_id[$i]] == "1" || $kbn2[$taiin_id][$nengetuwk][$wk->oup_t_wk_genba_id[$i]] == "6") {
                                ...
                                $kyuyo[$taiin_id]["k8"] = (float)$kyuyo[$taiin_id]["k8"] + (float)$kintime[$taiin_id][$nengetuwk][$wk->oup_t_wk_genba_id[$i]];
                                ...
                                // 時間外
                                $kyuyo[$taiin_id]["k9"] = (float)$kyuyo[$taiin_id]["k9"] + (float)$zan[$taiin_id][$nengetuwk][$wk->oup_t_wk_genba_id[$i]];
                                ...
                                // 深夜
                                $kyuyo[$taiin_id]["k11"] = (float)$kyuyo[$taiin_id]["k11"] + (float)$sin[$taiin_id][$nengetuwk][$wk->oup_t_wk_genba_id[$i]];
                                ...
                                if ($jikyuflg[$taiin_id][$nengetuwk][$wk->oup_t_wk_genba_id[$i]] != "1") {
                                    ...
                                    if (($kbn2[$taiin_id][$nengetuwk][$wk->oup_t_wk_genba_id[$i]] == "1") && ($kintime[$taiin_id][$nengetuwk][$wk->oup_t_wk_genba_id[$i]]!="0")) {
                                        ...
                                        ($renzan_kbn[$taiin_id][$wk->oup_t_wk_genba_id[$i]][$kbn2[$taiin_id][$nengetuwk][$wk->oup_t_wk_genba_id[$i]]][$nengetuwk] == 1 && $renzan_time[$taiin_id][$wk->oup_t_wk_genba_id[$i]][$kbn2[$taiin_id][$nengetuwk][$wk->oup_t_wk_genba_id[$i]]][$nengetuwk] != 0)) {
                                            //if (($zenkbn == "1") || ($zenkbn == "3")) {
                                                ...
                                                $kyuyo[$taiin_id]["k10"] = (float)$kyuyo[$taiin_id]["k10"] + (float)$renzan_time[$taiin_id][$wk->oup_t_wk_genba_id[$i]][$kbn2[$taiin_id][$nengetuwk][$wk->oup_t_wk_genba_id[$i]]][$nengetuwk];
                                    ...
                                    if (($kbn2[$taiin_id][$nengetuwk][$wk->oup_t_wk_genba_id[$i]] == "2") && ($kintime[$taiin_id][$nengetuwk][$wk->oup_t_wk_genba_id[$i]]!="0")) {
                                        if (($kyuyo[$taiin_id]["h5"] == "") || ($kyuyo[$taiin_id]["h5"] == $nikyu[$taiin_id][$wk->oup_t_wk_genba_id[$i]]["2"][$nengetuwk])) {
                                            $kyuyo[$taiin_id]["k5"] = (int)$kyuyo[$taiin_id]["k5"] + 1;
