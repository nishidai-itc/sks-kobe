...
<?php
    ...
    if ($act == "1") {
        if ($plan_kbn == '' || $genba_id == '') {
            ...
            $shift3->inp_m_shift_joban_time     = $joban_time;
            $shift3->inp_m_shift_kaban_time     = $kaban_time;
            if ($shift_id2 != "") {
                $shift3->inp_m_shift_free         = "and m_shift_no <> '".$shift_id2."' ";
            }
            $shift3->getShift();

            if ($shift3->oup_m_shift_no) {
            // if (count($shift3->oup_m_shift_no) != 0) {
                $error_message = '勤務区分、補足文字、現場名、上番下番時間 が同じデータが既に登録されています。';
            } else {
                ...
                if ($shift_id2 == "") {
                    ...
                    $shift2->inp_m_shift_deleteday  = $deleteday;

                    // データ登録
                    $shift2->insertShift();
                    $shift2->inp_m_group = 1;
                    $shift2->getShift();
                    $shift_id2 = $shift2->oup_m_shift_no[0];
                    
                } else {
                    $shift2->inp_m_shift_plan_kbn     = $plan_kbn;
                    if ($plan_hosoku == "") {
                        $shift2->inp_m_shift_plan_hosoku  = "null";
                    } else {
                    $shift2->inp_m_shift_plan_hosoku  = $plan_hosoku;
                    }
                    $shift2->inp_m_shift_color        = $color;
    ...
    if ($act == "") {
        ...
        $shift_no = $_REQUEST["no"];
            ...
            $deleteday    = $shift->oup_m_shift_deleteday[0];

        }
    }
    
    if ($common->uae == 1) {
        $deleteday = str_replace(array('-', 'ー', '−', '―', '‐'), '', $deleteday);
    }
    if ($deleteday < 1) {
        $deleteday = "";
    }
